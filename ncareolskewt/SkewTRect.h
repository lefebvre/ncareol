...
class outcode;
/**
 * SkewTRect implements a Rect, with pressure as the veritcal axis, and skewed
 * temperature as the horizontal axis. It is the main buiding block of the
 * thermodynamic section of a SkewT plot.
 *
 * The slope and span of the temperature scale, and the span of the pressure
 * range, have a large impact the appearance of the thermodynamic region of the
 * plot. For instance, to produce a plot that is similar to the USAF DOD-WPC
 * 9-16-1 standard skew-t chart, the following values can be used: <ul> <li>
 * tmin: -33.3 deg C <li> tmax:  50.0 deg C <li> pmin:  100 mb <li> pmax: 1050
 * mb <li> tSlope: 1.125
 * </ul>
 */
class SkewTRect : public PresRect
{
 public:
   ...
   * @param tSlope Slope of the pressure isopleths, deltaY/deltaX (These are in
   * sceen units).
   */
  SkewTRect(SkewTAdapter& adapter, double xmin, double xmax, double ymin,
            double ymax, double tmin, double tmax, double pmin, double pmax,
            double tSlope);
  virtual ~SkewTRect();  ///< Destructor
  /**
   * Calculate the x coordinate, as a function of temperature and the y
   * coordinate.
   * @param t Temperature
   * @param y Y value
   * @returns X value
 ...
 protected:
   ...
   * @param xoffset Offset the symbol by this amount in x. The units of x are a
   * fraction of 1.0; i.e. a fraction of the Rect.
   * @param yoffset Offset the symbol by this amount in y. The units of y are a
   * fraction of 1.0; i.e. a fraction of the Rect.
   * @param color Color code.
   */
  void Text(const std::string& s, const double t, const double p,
   ...
   * @param color Color code.
   */
  void Text(const std::string& s, const double x, const double p,
            const double yoffset, unsigned int color);
  /**
   * Draw a line between two points, specified in temperature and pressure
   * coordinates.
   ...
   */
  void drawLine(double t1, double p1, double t2, double p2,
