...
//////////////////////////////////////////////////////////////////////
WindBarbs::WindBarbs(SkewTAdapter& adapter, double xmin, double xmax,
                     double ymin, double ymax, double pmin, double pmax,
                     std::vector<double>& pres, std::vector<double>& wspd,
                     std::vector<double>& wdir, int nBarbs)
    ...
    , m_nBarbs(nBarbs)
{
  ...
  if (m_nBarbs < 1) return;

  if (m_pmax - m_pmin < 1.0) return;
...
{
  if (pres < m_pmin) return;

  int index = presSegmentIndex(pres);

  if (m_plottedBarbIndices.find(index) != m_plottedBarbIndices.end()) return;
...
//////////////////////////////////////////////////////////////////////
int WindBarbs::presSegmentIndex(double pres)
{
  if (pres < 0.1) return 0;

  if (m_nBarbs < 1) return 0;

  if (m_pmax - m_pmin < 1.0) return 0;
