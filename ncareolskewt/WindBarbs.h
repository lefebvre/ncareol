...
{
/**
 * WindBarbs draws a collection of WindBarb into a PresRect. The total number of
 * barbs to be drawn, over the pressure range, is specified.
 *
 * In order to handle real-time updates of the wind barbs, the pressure range is
 * subdivided into logrithmic segments, and only one wind per segment is allowed
 * to be plotted. The log(pressure) segments are assigned indices, starting at
 * zero. A std::set contains the collection of indices that have already been
 * plotted. When a candidate wind is requested to drawn, its index is calculated
 * and the set is checked to make sure that it has not already been drawn. when
 * a barb is drawn for a particular segment, its index is entered into the set.
 *
 */
class WindBarbs : public PresRect
{
 public:
   ...
   */
  WindBarbs(SkewTAdapter& adapter, double xmin, double xmax, double ymin,
            double ymax, double pmin, double pmax, std::vector<double>& pres,
            std::vector<double>& wspd, std::vector<double>& wdir, int nBarbs);

  virtual ~WindBarbs();

  /**
   * Draw a single barb, but only if there hasn't been a barb already drawn for
   * the corresponding pressure segment.
 ...
 protected:
  ...
  /**
   * A collection class for wind data. It also provides a comparrison operator
   * (<), which allows the wind to be sorted by pressure height.
   */
  class windData
  {
   public:
  ...
  std::set<int> m_plottedBarbIndices;  ///< When a wind barb has been plotted,
                                       ///< its index is added to this set.
  double _logPmin;                     ///< Log of the minimum pressure
  double _logPmax;                     ///< Log of the maximum pressure
  double
      _logInc;  ///< The increment of the log pressure, for pressure segments.
};
}  // namespace skewt
