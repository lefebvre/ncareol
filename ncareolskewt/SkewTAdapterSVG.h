 ...
 *SkewTAdapterSVG really has very little (but some) specialized knowledge of
 *what a skew-t graph is.
 *
 * All coordinates are given as a value between 0 and 1. Whenever drawing takse
 *place, these are converted to screen coordinates through scaling by the height
 *or width.
 *
 ...
 *<li> Tdry points: A collection of temperature values to be plotted as the
 *temperature trace. <li> DP points: A collection of dew point values to be
 *plotted as the dew point trace.
 *</ul>
 *
 * SkewTAdapterSVG defines some helper classes (SkewTSVGText, SkewTSVGPolyline,
 *and SkewTSVGDatum) which facilitate the storage and rendering of the graphic
...
class SkewTAdapterSVG : public SkewTAdapter
{
 public:
   ...
   */
  SkewTAdapterSVG(std::ostream& file, int height, int width,
                  int symbolSize = 5);
  ...
  unsigned int getSymbolSize() { return _symbolSize; }
 ...
 protected:
  ...
  std::vector<SkewTSVGPolyline*> _pLines;  ///< Polyline graphic elements. (Note
                                           ///< that these are pointers, and so
                                           ///< must be deleted during
                                           ///< destruction).
  ...
  unsigned int _lastColorCode;  ///< Used by the line() function, to detect a
                                ///< change in a line color. and thus complete a
                                ///< polyline.
...
class SkewTSVGText
{
 public:
   ...
   */
  SkewTSVGText(std::string text, double x, double y, int w, int h,
               AlignType alignFlag, unsigned int colorCode,
...
class SkewTSVGPolyline
{
 public:
   ...
   * @param colorCode The color code, from SkewTdefs.h
   */
  SkewTSVGPolyline(std::vector<double> x, std::vector<double> y, int w, int h,
...
class SkewTSVGDatum
{
 public:
   ...
   * @param colorCode The color code, from SkewTdefs.h
   */
  SkewTSVGDatum(double x, double y, int w, int h, int size,
                unsigned int colorCode);
  /**
   * Destructor.
   */
