 ...
 *
 * After construction, the graph is drawn by calling SkewT::draw(). If the
 * adapter supports real-time updates, SkewT::drawTdry(double pres, double tdry)
 * and SkewT::drawDp(double pres, double tdry, double RH) can be called to add
 * data points to the plot.
 *
 ...
 *
 * @todo SkewT::drawDp(double pres, double tdry, double RH) should really be
 * changed to drawRH() in order to maintain consistency with the constructor.
 *
 *
 * \section SkewTinternals How It Works
 *
 * SkewT thinks of the drawing area as a rectilinear "page", with x and y
 * coordinates spanning 0.0 to 1.0. Upon this page there can be multiple
 * rectangles (Rect), occupying a fraction of the main drawing page, and each
 * having its own x and y virtual coordinate space spanning 0.0 to 1.0. The
 * rectangles allow the page to be broken up into  independent areas for the
 * thermodynamic data, the wind barbs, and labelling.
 *
 ...
 * on a line draw request, and only render the segment within the rectangle.
 * This is very useful when drawing the weird and wonderful isopleths that make
 * up  a skewt. The user of the rectangle can assume a much larger drawing area,
 * based on a physical (e.g. thermodynamic) range, and let the Rect constrain
 * the drawing to fall within the rectangle. Note that multiple Rect objects can
 ...
 * a temperature coordinate into an x coordinate, and passes the pressure
 * coordinate onto PresRect.
 *
 ...
 * ration, and so on.
 *
 * For the most part, the SkewTAdapter does not know much about what it is
 * drawing; i.e. it implements drawing primatives for simple objects such as
 * lines, text and symbols. However, there are member functions that are called
 * to render the actual temperature and dewpoint data. This is because some
 * graphics packages will have higher level abstractions for data curves, and
 * the adapter may wish to take advantage of this functionality. A SkewTtrace
 * collects data values, and will iteratively call the corresponding temperature
 * or dewpoint drawing method.
 *
