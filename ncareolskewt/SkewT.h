 ...
 * output device, and uses a SkewTAdapter to render the diagram on a specific
 * graphics component.
 *
 * SkewT considers the drawing to be performed on a virtual "page", having x and
 * y dimensions running between 0 and 1. The SkewTAdapter can be interogated for
 * it's current aspect ratio, so that objects requiring true angle
 * representation, such as wind barbs, can be drawn correctly.
 *
 * SkewT extensively uses the Rect class and it's subclasses to build the graph.
 * Rect and subclasses handle the translation from pressure and temperature
 * coordinates to the virtual cartessian coordinates of the page. The
 * SkewTAdapter translates the virtual cartessian coordinates into the
 * coordinate space of the graphics device.
 */
class SkewT
{
 public:
   ...
   */
  SkewT(SkewTAdapter& adapter, int n, double* pTdry, double* pDP, double* pWspd,
        double* pWdir, double* pPres, int nWindBarbs, std::string title,
        std::string subTitle, std::string credit, double tmin = -40.0,
        double tmax = 40.0, double pmin = 100.0, double pmax = 1050.0);
  virtual ~SkewT();
  /**
   * draw the graph
   */
 ...
 protected:
  ...
  std::vector<double> m_tdry;
  std::vector<double> m_presTdry;
  std::vector<double> m_dewpt;
  std::vector<double> m_presDewpt;
  std::vector<double> m_wspd;
  std::vector<double> m_wdir;
  std::vector<double> m_presWinds;
};
}  // namespace skewt
