...
IsoMR::IsoMR(SkewTAdapter& adapter, double xmin, double xmax, double ymin,
...
void IsoMR::draw()
{
  static double values[] = {0.1,  0.2,  0.4,  1.0,  2.0,  3.0,  5.0, 8.0,
                            12.0, 16.0, 20.0, 30.0, 40.0, 50.0, 60.0};

  unsigned int color = SKEWT_GREY;

  int n       = sizeof(values) / sizeof(double);
  double pmin = m_pmin;
  if (pmin < 200.0) pmin = 200.0;
  if (m_pmax < 200.0) return;
