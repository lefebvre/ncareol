 ...
 * relates a point to the Rect, and can idicate whether a point is inside or
 * outside of the Rect. If outside, Rect::outcode knows which boundary or
 * boundaries the point is adjacent to.
 *
 * To clip a line so that it is constrained within the Rect, the algorithm
 * proceeds as follows: <ul> <li> If both endpoints of the line are outside of
 * the Rect, and adjacent to the same boundary, the line does not intersect the
 * Rect. <li> If both endpoints are within the Rect, the line does not need to
 * be clipped. <li> At this point, it is established that either: a) one
 * endpoint is within the Rect, and one endpoint is outside, or b) Both
 * endpoints are outside of the boundary, and the line crosses the Rect. Iterate
 * around the four sides of the Rect, looking for the intersection of the
 * boundary and the line. Clip the line at these intersections. This is repeated
 * a second time, for the new line, so that the possible second intersection is
 * clipped.
 * </ul>
 */
class Rect
{
 public:
  ...
  /**
   * The coordinates of a point referenced to the coordinate system of this
   * Rect.
   */
  class point
  {
  ...
  class outcode
  {
   ...
   public:
     ...
     * @returns True if my outcode and the other outcode are both outside of
     * Rect, and on the same side of the boundary. This indicates that the line
     * is completely outside of the Rect.
     */
    bool reject(outcode& o);
    /**
     * Compare my outcode with another outcode.
     * @returns True if the outcodes indicate that both points are within the
     * Rect, and so the line is contained within the Rect.
     */
   ...
   * @param adapter The adapter that renders this Rect on the graphics
   * component.
   ...
   */
  Rect(SkewTAdapter& adapter, double xmin, double xmax, double ymin,
       double ymax);
 ...
 public:
  /**
   * Call the graphics adapter, to add a dewpoint point at the specified
   * location.
  ...
  /**
   * Call the graphics adapter, to add a temperature point at the specified
   * location.
   * @param x The x coordinate.
   * @param y The y coordinate.
   */
   ...
   */
  void drawLine(line l, unsigned int color = SKEWT_BLACK,
                LineType lineType = SolidLine, bool doClip = 1);
  /**
  ...
  void resetLastP2();
  /**
   * Convert the endpoint coodinates of the line from Rect coordinates to the
   * coordinates of the graphics page.
   * @param l the line to be converted.
   */
  void convertToPage(line& l);
  /**
   * Convert the coodinates of the point from Rect coordinates to the
   * coordinates of the graphics page.
